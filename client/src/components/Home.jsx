...
function Home() {
    ...
    const handleLogout = async () => {
        try {
            let res = await axios.get("/api/logout", { baseURL: import.meta.env.VITE_BACKEND_URL, withCredentials: true });
            if (res.status === 200) {
                setUser(null);
                nav("/login");
            } else {
    ...
    const handleJoinRoom = async () => {
        try {
            let res = await axios.get(`/api/room/${roomID}`, { baseURL: import.meta.env.VITE_BACKEND_URL, withCredentials: true });
            if (res.data.success) {
    ...
    const handleUser = async () => {
        ...
        setLoading(true);
        try {
            let res = await axios.get("/api/user", { baseURL: import.meta.env.VITE_BACKEND_URL, withCredentials: true });
            if (res.data.success) {
